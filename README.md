...
it makes sound immediately — or hit **Vocals** to load one of the embedded vocals
...
### Built-in vocal sources

80 royalty-free vocal sources ship in `examples/` so you have something to load
straight away, from pop/EDM hooks and chops to breath fills, choir hits, pads and
mouth percussion. They are synthetic/formant-generated sources made for chopping
and keyboard mapping, not uncleared commercial samples.

- `examples/vocal_chop_demo.wav` — a vowel-formant "synth vocal" melody (try the
  Pitch / Formant knobs and Grid slicing).
- `examples/vox_dancepop.wav`, `examples/vox_futurehouse.wav`,
  `examples/vox_edmdrop.wav`, `examples/vox_bigroom.wav` — chart-pop and EDM
  starting points.

They are generated by `tools/generate_vocal_chops.py` (pure Python, no deps) —
re-run it to regenerate the embedded vocal set.
